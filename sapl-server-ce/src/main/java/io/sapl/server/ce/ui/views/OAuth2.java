...
public class OAuth2 extends VerticalLayout implements BeforeEnterObserver {
    ...
    void init() {
        ...
        Anchor loginLink = new Anchor("/oauth2/authorization/keycloak", "Login with Oauth2");
        loginLink.getElement().setAttribute("router-ignore", true);

        Button loginButton = new Button("Login mit Keycloak", click -> {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        if (authenticatedUser.get().isPresent()) {
            // Already logged in
            event.forwardTo("/oauth2/authorization/keycloak");
        }
    }
}
